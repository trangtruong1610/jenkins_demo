pipeline{
    ...
    stages{
        ...
        stage('Functional regression tests') {
            steps {
                sh "docker run --shm-size=1g -e BROWSER=chrome -e ROBOT_TESTS_DIR=/opt/robotframework/tests/test.robot -v $WORKSPACE/robot-tests/test.robot:/opt/robotframework/tests:Z ppodgorsek/robot-framework"
            }
        }
